...
Supabase の `academia_domains` テーブルで管理されている大学ドメインのユーザーには、`isAcademiaEmail` 判定によって自動的にサブスクリプションが付与される仕組み（api/me.ts）が存在します。

ドメインの追加・削除・無効化は Supabase Dashboard から `academia_domains` テーブルを操作して行います。`is_active` を `false` にするとドメインが無効化されます。変更は最大5分で反映されます（インメモリキャッシュのTTL）。
